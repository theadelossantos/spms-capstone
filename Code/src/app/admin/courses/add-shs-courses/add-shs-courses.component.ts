...
@Component({
  selector: 'app-add-shs-courses',
  templateUrl: './add-shs-courses.component.html',
  styleUrls: ['./add-shs-courses.component.css']
})
export class AddShsCoursesComponent {
  ...
  addSubject(){
    if(this.ValidationFormUser.valid){
      ...
      this.authService.addsHsSubject(subjectData).subscribe(
        (response) => {
          this.showAlert = true;
